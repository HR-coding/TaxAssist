...
class TestITR2Models(unittest.TestCase):
    def test_vda_and_cfl_present(self):
        from app.models.tax_ledger import ITR2Ledger
...
class TestStateModels(unittest.TestCase):
    def test_itr1_checklist_defaults(self):
        from app.models.state import ITR1_CHECKLIST_DEFAULTS
    ...
    def test_itr2_checklist_defaults(self):
        from app.models.state import ITR2_CHECKLIST_DEFAULTS
    ...
    def test_notification_context_metadata(self):
        from app.models.state import NotificationBlock, ContextMetadata
...
class TestDecider(unittest.TestCase):
    ...
    def test_missing_pan_aadhaar(self):
        from app.engine.decider import evaluate_itr1_next_step
        state = {
    ...
    def test_itr2_uses_different_milestone_keys(self):
        from app.engine.decider import evaluate_itr2_next_step
        state = {
            "portal_prerequisites": {
...
class TestStateTrackerService(unittest.TestCase):
    def setUp(self):
        ...
        self.patcher = patch("app.services.state_tracker_service.db", mock_db)
        self.patcher.start()
    ...
    def test_create_itr1_state_has_correct_defaults(self):
        from app.services.state_tracker_service import create_state
    ...
    def test_update_state_writes_back(self):
        from app.services.state_tracker_service import update_state
        with patch("app.services.state_tracker_service.db") as mock_db:
...
class TestITRService(unittest.TestCase):
    def test_create_itr1_uses_correct_ledger(self):
        with patch("app.services.itr_service.db") as mock_db, \
             patch("app.services.itr_service.compute_calculated_fields", return_value={}):
            mock_db.itr_records.insert_one.return_value = MagicMock()
            from app.services.itr_service import create_itr
    ...
    def test_update_itr_triggers_recalc(self):
        ...
        with patch("app.services.itr_service.db") as mock_db, \
             patch("app.services.itr_service.compute_calculated_fields", return_value={"salary_income.net_salary_income.value": 725000}) as mock_calc:
            mock_db.itr_records.find_one.return_value = mock_full_doc
            from app.services.itr_service import update_itr
...
class TestITRMapper(unittest.TestCase):
    ...
    def test_scalar_field_sets_value(self):
        ...
        with patch("app.services.itr_mapper.update_itr") as mock_update:
            from app.services.itr_mapper import apply_extraction_to_itr
    ...
    def test_array_field_pushes_item(self):
        ...
        with patch("app.services.db.db", mock_db), \
             patch("app.services.itr_mapper.update_itr"):
            from app.services.itr_mapper import apply_extraction_to_itr
...
class TestGateway(unittest.TestCase):
    ...
    def _get_client(self, state_doc=None):
        ...
        with patch("app.api.gateway.db") as mock_db:
    ...
    def test_intent_mismatch_returns_403_and_blocks(self):
        ...
        with patch("app.api.gateway.db") as mock_db, \
             patch("app.api.gateway.determine_next_action", return_value="VERIFY_INCOME"):
    ...
    def test_valid_request_returns_200(self):
        ...
        with patch("app.api.gateway.db") as mock_db, \
             patch("app.api.gateway.determine_next_action", return_value="VERIFY_PAN"):
            mock_db.state_tracker.find_one.return_value = state
            mock_db.itr_records.update_one.return_value = MagicMock()
            mock_db.document_registry.update_many.return_value = MagicMock()
            client = TestClient(app)
...
class TestOrchestratorITR1(unittest.TestCase):
    ...
    def _build_state(self, prereqs_done=False, doc_verified=False, milestones_done=False):
        from app.models.state import ITR1_CHECKLIST_DEFAULTS, ITR1_MILESTONE_DEFAULTS
        prereqs = {
...
class TestOrchestratorITR2(unittest.TestCase):
    ...
    def _build_itr2_state(self, all_verified=False):
        from app.models.state import ITR2_CHECKLIST_DEFAULTS, ITR2_MILESTONE_DEFAULTS
        prereqs = {
            "pan_aadhaar_linking_status": {"status": "VERIFIED"},
            "bank_account_prevalidation": {"status": "VERIFIED"},
            "part_a_general_info": {"status": "VERIFIED"}
...
class TestADKTools(unittest.TestCase):
    ...
    def test_calculate_itr1_tax_tool_nested(self):
        from app.adk.tools import calculate_itr1_tax_tool
    ...
    def test_retrieve_tax_rules_tool(self):
        from app.adk.tools import retrieve_tax_rules_tool
    ...
    def test_check_state_tool_calls_mcp(self):
        with patch("app.adk.tools.check_state_mcp", return_value={"user_id": "u1", "next_action": {"action": "VERIFY_PAN_AADHAAR_LINKING_STATUS"}}) as mock_mcp:
            from app.adk.tools import check_state_tool
