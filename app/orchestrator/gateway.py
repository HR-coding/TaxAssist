...
def execute_tool(
    ...
    x_agent_verifier_code: str = Header(..., alias="X-Agent-Verifier-Code")
):
    # 1. Identity Verification Gate
    secret_key = os.getenv("AGENT_SECRET_KEY", "")
    if not secret_key:
        logger.warning("AGENT_SECRET_KEY is not configured in the environment.")
    
    # Constant-time comparison using hmac
    if not hmac.compare_digest(x_agent_verifier_code.encode("utf-8"), secret_key.encode("utf-8")):
